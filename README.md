...
미배포 후속 변경에는 패널 이동 완료 후 버튼을 측정하는 테스트 수정과, 날씨 검증 실패 단계를 제한된 서버 코드로 구분하는 진단이 있습니다. 날씨 응답 수용 조건과 공개 응답은 바꾸지 않으며, 임의 내부 오류 메시지가 서버 로그에 남는 경로도 닫습니다. 실제 원인 해결 여부는 독립 리뷰·CI·Preview 재현 후 판단합니다.
