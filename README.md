...
> 현재 상태: 원격 `develop`의 검토된 SHA `1dc01b1`이 Preview에 배포됐습니다. 모바일 지도 가림과 경로 요약 수정은 독립 리뷰 및 실제 지도 화면 검사를 통과했지만, 이후 연결 검사에서 경로 계산이 `502`로 실패해 전체 Preview 검증은 미완료입니다. 실패 항목을 안전하게 구분하는 진단만 추가했으며 원인 수정은 아직 아닙니다. Preview `plan-route`는 ACTIVE v9·JWT 검증 유지, 나머지 네 함수는 v8이며 DB는 변경하지 않았습니다. 명시적 승인 후 연결 검사 1회를 실행했으나 `RESULT_CODE` 검사에서 거절되어 계획 저장·컬렉션 생성·공유 발행 전에 실패했습니다. 정리할 테스트 계획·컬렉션·공유는 생성되지 않았으며, 정확한 공급자 결과 코드 값은 아직 확인되지 않았습니다. 유료 API·자동 결제·Production은 변경하지 않았습니다.
