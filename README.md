...
> 현재 상태: Preview 게이트를 진행 중입니다. 이메일 없는 Kakao OIDC 직접 연동, 실제 Preview 로그인, 최초 관리자 등록, 카카오 지도 렌더링과 실제 장소 검색까지 확인했습니다. 장소 URL은 정확한 Kakao 호스트만 HTTPS로 정규화하며, 실제 경로가 없을 때 지도 지점을 직선으로 잇지 않습니다. 현재 로컬 수정은 세 후보와 자동 와인딩 탐색을 제거하고, 사용자가 지정한 모든 지점을 지나는 안전 `RECOMMEND` 경로 1개만 계산·저장·공유합니다. 유료 API나 자동 결제는 활성화하지 않으며 Production은 변경하지 않았습니다.
