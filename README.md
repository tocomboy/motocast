...
지인 라이더를 위한 국내 24시간 미만 오토바이 경로·시간대별 날씨 계획 PWA입니다. 사용자가 정한 장소와 커스텀 와인딩 경유지를 지나는 오토바이 안전 추천 경로 1개의 실제 도로 형상, 예상 복귀 시각, 구간별 통과 시각과 기상청 예보를 보여줍니다.

> 현재 상태: 단일 추천 경로 변경은 검토된 `develop` SHA로 Preview에 배포되었습니다. 실제 Preview에서 Kakao 로그인·지도·장소 검색과 사용자가 지정한 모든 지점을 지나는 안전 `RECOMMEND` 경로 1개의 도로 형상·저장까지 확인했습니다. 기상청 호출은 안전한 일반 오류로 종료되어 상류 상태만 비밀 없이 구분하는 관측 보강과 실제 예보 재검증을 진행 중이며, 컬렉션·공유까지 이어지는 전체 Preview 게이트는 아직 GREEN이 아닙니다. 유료 API나 자동 결제는 활성화하지 않았고 Production은 변경하지 않았습니다.
