...
> 현재 상태: 원격 `develop`의 검토된 SHA `57cfa83`가 Preview에 배포되어 하나의 정렬 가능한 경유지 목록, 완전한 코스 컬렉션, 실제 Kakao 도로 경로와 ETA 기반 기상청 예보, 저장·공유·회수·재발행의 연결형 검증까지 통과했습니다. Preview Supabase는 11개 migration이 일치하고 Edge Function 5개가 모두 ACTIVE v8입니다. 로컬 `develop`의 UI/accessibility 후속 후보는 지도 정보와 경로 요약을 지도 밖으로 옮기고 모바일 가독성과 터치 영역을 보완했지만, 아직 독립 고정-SHA 리뷰·CI·Preview 재검증 전이라 게시되지 않았습니다. 유료 API나 자동 결제는 활성화하지 않았고 Production은 변경하지 않았습니다.
