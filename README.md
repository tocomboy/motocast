...
> 현재 상태: 단일 추천 경로 변경은 검토된 `develop` SHA로 Preview에 배포되어 Kakao 로그인·지도·장소 검색과 실제 도로 형상·저장까지 확인했습니다. 다만 그 Preview는 종류별 입력을 따로 합쳐 방문 순서를 바꿀 수 있는 문제가 있어, 하나의 정렬 가능한 경유지 목록으로 고치는 후속 변경을 로컬에서 검증 중입니다. 기상청 호출도 아직 안전한 일반 오류로 종료되므로 컬렉션·공유까지 이어지는 전체 Preview 게이트는 GREEN이 아닙니다. 유료 API나 자동 결제는 활성화하지 않았고 Production은 변경하지 않았습니다.
