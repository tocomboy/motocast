...
> 현재 상태: 원격 `develop`의 검토된 SHA `52f9cb5`가 Preview에 배포됐습니다. 모바일 지도 가림과 경로 요약 수정은 독립 리뷰 및 실제 지도 화면 검사를 통과했습니다. 경로 계산 실패의 서버 진단은 카카오 결과 코드 `106`(요청 도착 지점 주변 교통 장애)으로 확인됐지만, 식사·휴식별 분할 요청 중 어느 지점인지 아직 특정하지 못해 전체 Preview 검증은 미완료입니다. 기존 입력과 전체 도착지만 바꾼 비교 검사는 모두 계획 저장·컬렉션 생성·공유 발행 전에 실패했고 해당 리소스의 정리 대상은 없었습니다. Preview `plan-route`는 ACTIVE v10·JWT 검증 유지, 나머지 네 함수는 v8입니다. 필요한 후속 Preview 테스트는 사용자 승인 범위이며, 안전 옵션·DB·Production·유료 API·자동 결제는 변경하지 않았습니다.
