...
> 현재 상태: Preview 게이트를 진행 중입니다. 이메일 없는 Kakao OIDC 직접 연동, 실제 Preview 로그인, 최초 관리자 등록, 카카오 지도 활성화와 지도 렌더링까지 확인했습니다. 첫 실제 장소 검색에서 카카오의 공식 `http://place.map.kakao.com/...` 응답을 브라우저의 HTTPS 전용 계약이 거절하는 문제가 확인되어, 서버가 정확한 Kakao 호스트만 검증해 HTTPS로 정규화하도록 수정·검증 중입니다. 실제 경로가 없을 때는 지도 지점을 직선으로 잇지 않고, 경로 계산 성공 뒤에만 Kakao Mobility가 반환한 도로 좌표 전체를 표시합니다. 유료 API나 자동 결제는 활성화하지 않으며 Production은 변경하지 않았습니다.
...
- 실제 카카오 장소 URL 정규화와 도로 좌표 전용 지도 선 수정을 리뷰·재배포하고 장소 검색을 재검증
