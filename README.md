...
> 현재 상태: 원격 `develop`의 검토된 SHA `92dc2de`가 Preview에 배포됐고 PR·develop CI 모두 통과했습니다. 모바일 지도 가림·경로 요약 수정과 버튼 크기 검사 경합 수정은 검증됐습니다. 기존 테스트 경로는 점심→경유→휴식 구간에서 카카오 코드 `106`으로 거절됐지만, 휴식지만 바꾼 비교 경로는 실제 계산·저장·지도 표시가 성공했습니다. 같은 비교 입력의 최신 재현은 기상청 응답의 발표 날짜·시각 또는 형식 불일치(`KMA_INVALID_RESPONSE BASE_BINDING`)로 실패했고 테스트 계획은 정리했습니다. 컬렉션·공유 발행까지 포함한 전체 Preview 검증은 미완료입니다. Preview 함수는 `plan-route` v11, `weather-timeline` v9, 나머지 세 함수 v8이며 기존 JWT 설정을 유지합니다. 필요한 후속 Preview 테스트는 사용자 승인 범위이며, 안전 옵션·DB·Production·유료 API·자동 결제는 변경하지 않았습니다.

배포된 날씨 진단은 응답 수용 조건과 공개 응답을 유지하면서 임의 내부 오류 메시지가 서버 로그에 남는 경로를 닫았습니다. 다음 단계는 발표 기준의 타입·형식·값 중 무엇이 다른지 안전하게 구분하는 것입니다. 현재 진단만으로 잘못된 예보를 허용하거나 응답 값을 추측해 보정하지 않습니다.

미배포 후속 진단은 날짜·시각 각각의 타입, 형식, 값 불일치와 숫자 표현의 동등성을 구분합니다. 숫자가 요청 문자열과 동등해도 아직 응답을 수용하지 않으며, 검토·CI·Preview 재현 후에만 원인과 수정 방향을 판단합니다.
