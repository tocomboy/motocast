...
> 현재 상태: 원격 `develop`의 검토된 SHA `c926198`이 Preview에 배포됐고 PR·develop CI 모두 통과했습니다. 모바일 지도 가림·경로 요약 수정과 버튼 크기 검사 경합 수정은 검증됐습니다. 기존 테스트 경로는 점심→경유→휴식 구간에서 카카오 코드 `106`으로 거절됐지만, 휴식지만 바꾼 비교 경로는 실제 계산·저장·지도 표시가 성공했습니다. 같은 비교 입력의 최신 재현은 기상청 응답의 발표 시각 값 불일치(`KMA_INVALID_RESPONSE BASE_TIME_MISMATCH`)로 실패했고 테스트 계획은 정리했습니다. 컬렉션·공유 발행까지 포함한 전체 Preview 검증은 미완료입니다. Preview 함수는 `plan-route` v11, `weather-timeline` v10, 나머지 세 함수 v8이며 기존 JWT 설정을 유지합니다. 필요한 후속 Preview 테스트는 사용자 승인 범위이며, 안전 옵션·DB·Production·유료 API·자동 결제는 변경하지 않았습니다.

배포된 날씨 진단은 응답 수용 조건과 공개 응답을 유지하면서 임의 내부 오류 메시지가 서버 로그에 남는 경로를 닫았습니다. 실제 실패는 날짜나 타입·형식 문제가 아닌 발표 시각 값 불일치로 좁혀졌습니다. 다음 단계는 공식 발표·갱신 계약과 요청·응답 시각 관계를 대조하는 것이며, 현재 진단만으로 잘못된 예보를 허용하거나 응답 값을 추측해 보정하지 않습니다.

최신 연결 테스트는 `PASS 0 / FAIL 1 / SKIP 0 / SETUP_OR_IMPORT_FAILURE 0`이며, 날씨 이후 컬렉션 저장과 공유 발행·회수·재발행은 미실행입니다. 별도 정리 훅에서 정확한 테스트 계획을 삭제했고 정리 실패는 없었습니다. 발표 시각의 실제 값·차이 및 불일치 원인은 아직 확인하지 않았습니다.

## 고정된 제품 원칙

개발 중인 후속 진단은 이미 거절된 응답의 발표 관계를 최대 1,000개 항목까지 한 번에 요약합니다. 발표가 이전·일치·이후인지, 여러 발표가 섞였는지와 중간 갱신 형태를 고정 범주로만 기록하며, 검사 범위를 넘으면 부분 결과라고 표시합니다. 날짜·시각·위치·예보 값은 기록하지 않고 기존 오류와 수용 조건은 그대로 유지합니다. 아직 이 진단의 Preview 배포나 실제 관측은 완료하지 않았습니다.
