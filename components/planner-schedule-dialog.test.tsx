...
describe("PlannerScheduleDialog", () => {
  ...
  it("requires an explicit hour and minute and rejects a past time chosen for today", async () => {
    ...
    await act(async () => buttonWithText(renderer, "날짜와 출발 시각 선택")!.props.onClick());
    ...
    await act(async () => buttonWithText(renderer, "08")!.props.onClick());
    await act(async () => renderer.root.findByProps({ className: "schedule-time-rows" }).findAllByType("button")[1].props.onClick());
    await act(async () => buttonWithText(renderer, "00")!.props.onClick());
    await act(async () => buttonWithText(renderer, "일정 적용")!.props.onClick());
