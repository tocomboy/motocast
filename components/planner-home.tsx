...
export function PlannerHome({ connected, busy, onNewRoute, onCollections, collections }: { connected: boolean; busy: boolean; onNewRoute: () => void; onCollections: () => void; collections?: ReactNode }) {
  return (
    <section className="planner-home" aria-labelledby="planner-home-title">
      ...
      {connected && collections ? <div className="planner-home-collections"><div className="planner-home-section-heading"><div><h2>최근 저장한 경로</h2><p>경로를 고른 뒤 출발 날짜·시간을 정하세요.</p></div><button type="button" onClick={onCollections}>전체 보기</button></div>{collections}</div> : null}
    </section>
  );
}
