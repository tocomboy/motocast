...
test("renders model, outside-window, and stale states without changing route order", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "구간별 날씨" })).toBeVisible();
  ...
  const weatherDetails = page.locator(".weather-detail");
  await weatherDetails.locator("summary").click();
  await expect(weatherDetails).toContainText("초단기예보");
  await expect(weatherDetails).toContainText("단기예보");
  await expect(weatherDetails).toContainText("예보 범위 밖");
  await expect(page.locator(".summary-route-order")).toContainText("출발");
  await expect(page.locator(".summary-route-order")).toContainText("복귀");
...
test("renders a schema version 3 share with one recommended route and no candidate selector", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: current.trip.title })).toBeVisible();
  await expect(page.locator(".summary-route-order")).toContainText(current.trip.origin.label);
  await expect(page.locator(".summary-route-order")).toContainText(current.trip.destination.label);
  await expect(page.getByText("도착 시각은 교통 상황에 따라 달라질 수 있는 추정값입니다.")).toBeVisible();
...
test("renders an immutable schema version 1 share with its historical return fields", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: legacy.trip.title })).toBeVisible();
  await expect(page.locator(".summary-route-order")).toContainText(legacy.trip.origin.label);
  await expect(page.locator(".summary-route-order")).toContainText(legacy.trip.destination.label);
  await expect(page.getByText("도착 시각은 교통 상황에 따라 달라질 수 있는 추정값입니다.")).toBeVisible();
  await expect(page.getByRole("heading", { name: "구간별 날씨" })).toBeVisible();
  await expect(page.getByText("구간별 도착 시간")).toBeVisible();
});
