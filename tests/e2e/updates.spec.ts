...
test.describe("public update notes", () => {
  test("shows one current-version announcement per server identity decision", async ({ page }) => {
    ...
    await expect(dialog.getByText("경유지 최대 5개와 자동차전용도로 제외 안내를 강조했어요.")).toBeVisible();
  ...
  test("opens from the global version footer and shows the current release history", async ({ page }) => {
    ...
    const expectedVersions = [currentVersion, "0.4.0", "0.3.0", "0.2.3", "0.2.2", "0.2.1", "0.2.0", "0.1.0"];
    ...
    for (const [index, version] of expectedVersions.entries()) {
      ...
      await expect(releases.nth(index).getByText("현재 버전", { exact: true })).toHaveCount(index === 0 ? 1 : 0);
    }
