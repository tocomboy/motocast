...
drop function if exists public.test_consume_kakao_oidc_handoff(text);

create function public.test_consume_kakao_oidc_handoff(target_hash text)
returns text
language plpgsql
set search_path = public, pg_temp
as $$
begin
  return public.consume_kakao_oidc_handoff_internal(target_hash);
exception when others then
  return sqlerrm;
end;
$$;
revoke all on function public.test_consume_kakao_oidc_handoff(text) from public, anon, authenticated;
grant execute on function public.test_consume_kakao_oidc_handoff(text) to service_role;
...
select dblink_send_query('oidc_c1', format(
  'select public.test_consume_kakao_oidc_handoff(%L)', token_hash
...
insert into public.kakao_oidc_handoffs(token_hash, encrypted_payload, created_at, expires_at)
values (
...
insert into tap_results values
  ...
  (
    not has_function_privilege('anon', 'public.consume_kakao_oidc_handoff_internal(text)', 'EXECUTE')
    and not has_function_privilege('authenticated', 'public.consume_kakao_oidc_handoff_internal(text)', 'EXECUTE')
    and has_function_privilege('service_role', 'public.consume_kakao_oidc_handoff_internal(text)', 'EXECUTE'),
    'handoff consume RPC is service-role only'
  );

select dblink_disconnect('oidc_c1');
select dblink_disconnect('oidc_c2');
drop function public.test_consume_kakao_oidc_handoff(text);
delete from public.kakao_oidc_handoffs;
