...
- Status: `CONFIRMED`
- Decision: Use Kakao authorization-code OpenID Connect directly instead of Supabase Auth's hosted Kakao `signInWithOAuth()` start endpoint. Request only `openid`, optional `profile_nickname`, and optional `profile_image`; never request `account_email`. A public, JWT-exempt Supabase Edge Function owns the exact allowlisted redirect, Client Secret, CSRF `state`, and a per-attempt raw nonce whose SHA-256 value is sent to Kakao. The Kakao ID token, access token, and raw nonce cross to the application only through a short-lived, encrypted, single-use handoff whose plaintext bearer appears only in a URL fragment and is immediately removed. The application then calls Supabase `signInWithIdToken({ provider: "kakao" })`, including the access token and raw nonce, before applying the existing invitation and membership gate. Supabase Kakao remains enabled with `Allow users without an email`; Kakao OpenID Connect remains enabled. Failure at any state, nonce, handoff, token, invitation, or membership check fails closed and clears temporary authentication state.
...
- Verification: authorize URL contains `openid`, `profile_nickname`, and `profile_image` but not `account_email`; state cookie and return target tests; hashed nonce and `signInWithIdToken` nonce/access-token propagation; malformed/expired/replayed/cross-origin handoff denial; token exchange error redaction; invitation/member/revoked flows; hosted Preview login and Auth identity readback.
...
- Local database: after renewed exact Docker/workdir/port readback and explicit user approval, only the disposable PostgreSQL 17 database at `127.0.0.1:54322` was reset from empty. All six migrations, including `20260831213000_email_free_kakao_oidc.sql`, applied in order and the latest migration reapplied successfully. Final pgTAP results are Auth/RLS/budget `31/31`, plan/collection/share/weather `53/53`, invitation/budget/share concurrency `7/7`, collection concurrency `4/4`, route finalization concurrency `10/10`, OIDC handoff concurrency/ACL `8/8`, and live ACL allowlist `111/111`: total `224/224`, `not ok 0`. The OIDC two-consumer case proves exactly one ciphertext return and one `OIDC_HANDOFF_INVALID`; service role retains no direct application-table access and has exactly seven reviewed internal RPC grants. Neither hosted project was reset or mutated.
- Writer verification for the uncommitted `AUTH-004` delta: a fresh writable `/tmp` copy passes `npm ci` (`401` packages), dependency-tree readback, ESLint, TypeScript, Vitest (`40 files / 209 tests`), and the Next.js production build. Deno checks all five Edge Function entrypoints. Final database evidence totals `224/224` as recorded above. One earlier sandboxed `npm ci` attempt failed setup when registry requests returned `EAI_AGAIN`, and mounted-worktree TypeScript/Vitest attempts failed setup because Windows-mounted build caches were read-only; neither is counted as a product test failure or hidden as a pass. Final diff/sensitive-value scans remain required before a fixed SHA is created; no independent review of `AUTH-004` has occurred yet.
