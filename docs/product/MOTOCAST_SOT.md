...
- Writer verification for initial fixed SHA `33b6fc36e74ef13ae5901311f2e1b58a94cae76c`: a fresh writable `/tmp` copy passed `npm ci` (`401` packages), dependency-tree readback, ESLint, TypeScript, Vitest (`40 files / 209 tests`), and the Next.js production build; Deno checked all five Edge Function entrypoints and the then-current database passed `224/224`. Independent review found browser-unbound login handoff `HIGH 1`, fragment-removal failure `HIGH 1`, database clock/ACL `MEDIUM 2`, callback recovery `MEDIUM 1`, and profile/documentation `LOW 3`; that SHA is rejected. The correction work adds app-origin browser binding, advancing-clock expiry, complete RPC ACL/RLS tests, OIDC `picture` preservation, exception-safe fragment removal, and a bounded callback timeout. The writable clean copy now passes ESLint with zero warnings, TypeScript, Vitest (`40 files / 212 tests`), and the Next.js production build including both Kakao API routes; Deno checks all five Edge Function entrypoints, final DB is `233/233`, `git diff --check` passes, and the names-only sensitive-value scan is empty. A new fixed SHA and independent delta reviews remain pending. Earlier sandbox/network/mounted-cache setup failures remain recorded and are not counted as product failures.
