...
- Verification: exactly one provider invocation per split chunk; fixed safety parameters and absent `alternatives`; ordered mandatory point matching; dwell/return/24-hour boundaries; DB-side road distance/duration totals, numeric Korea-bounded vertices, endpoint/road continuity, and missing-field rejection; one-draft atomic finalization and concurrency; one route row; legacy three-route finalization/read compatibility; schemaVersion 3 singular share plus schemaVersion 1/2 parsing; no candidate UI at mobile/desktop; real Preview route, geometry, ETA weather, collection, and share smoke.
