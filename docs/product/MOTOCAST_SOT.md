...
- Fifth-review working correction on 2026-09-01: fixed SHA `af5f05ce71c4e527cb810982d168a40af28f2114` is rejected for release even though correctness, security, data-integrity, and route-safety approved it with zero findings. UI/accessibility and operations independently found `MEDIUM 1`: up to twenty full-width omission rows remained inside an unbounded absolute map legend, so small shared maps could clip them or let them cover the route summary; the existing browser fixture represented only five plain role rows. The current correction keeps only compact role keys in the ready-map overlay and renders every `선택 경로 미통과` item in a normal-flow notice after the fixed-height map. The notice therefore remains present during SDK loading or failure and grows with the supported twenty-waypoint boundary instead of being clipped by map overflow. Focused Vitest passes `13/13`, including SDK-error visibility and all twenty items; focused production Chromium passes `10/10`, including containment and no horizontal overflow at 320x800, 390x844, 820x1180, and 1440x900. The mounted worktree first produced `SETUP_OR_IMPORT_FAILURE 2` because Vitest/TypeScript could not write caches on the Windows mount; the `/tmp` rerun is the product evidence. One initial unit authoring run failed two text-extraction assertions before correction (`FAIL 2`), and one sandboxed browser start was blocked by local-port `EPERM` (`SETUP_OR_IMPORT_FAILURE 1`) before the permitted identical command passed. Full writer verification, a new fixed SHA, six-axis delta review, hosted CI, and connected Preview remain pending. Production remains unchanged.
