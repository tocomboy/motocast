...
- GitHub: public repository; default `develop`; `main` required checks `verify` and `develop-only`; PR required with zero approvals; administrators and conversation resolution enforced; force pushes and deletion disabled. Remote `develop` SHA `b9b96d7737fa12a74f5bf7d1dba088e8e056fb2a` has a successful CI run.
- Vercel: project `tocomboys-projects/motocast`, GitHub repository linked, Production Branch `main`, Node.js `20.x`, and `ssoProtection.deploymentType=preview`. Preview requires Vercel Authentication while Production remains outside that platform gate. Preview deployment `dpl_BMv1sT2QLctfDqB5gNLUCo5NQZkW` is Ready at the fixed `develop` alias for SHA `b9b96d7`, but it predates the final environment cleanup and must be rebuilt. The unchanged import-time Production deployment remains Ready; no Production redeploy or promotion occurred.
...
- Local database: after renewed exact Docker/workdir/port readback and explicit user approval, only the disposable PostgreSQL 17 database at `127.0.0.1:54322` was reset from empty. All five migrations applied in order and `20260830224500` reapplied successfully. The final combined Auth/RLS/budget, complete ACL, and plan/collection/weather/share run passes `189/189`; separate two-connection suites pass collection/FK `4/4`, invitation/budget/share publication `7/7`, and route finalization/rollback `10/10`. Reapplying the latest migration after the concurrency suites succeeds, and the expanded service-role allowlist readback including disposable dblink functions passes `146/146`. Earlier test-harness failures remain recorded in the prior review round and are not counted as final pass. Neither hosted project was reset or mutated.
- Writer verification for correction commits through `232281e`: a fresh `/tmp` copy excluding `.git`, dependency/build output, and Vercel metadata passes `npm ci`, ESLint, TypeScript, Vitest (`36 files / 194 tests`), and the Next.js production build. Cached Deno 2.9.6 checks all four Edge Function entrypoints, `git diff --check` passes, and the names-only sensitive-pattern scan reports only expected SQL privilege identifiers rather than secret values. A mounted-worktree focused Vitest attempt had one setup error with zero tests executed because Vite could not write its temporary config under the Windows-mounted `node_modules`; the writable clean copy is the product verdict. All five independent review axes approve implementation SHA `2250d7e4` with zero findings; Preview provider/browser verification remains pending.
