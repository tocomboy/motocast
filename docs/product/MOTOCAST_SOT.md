...
- GitHub: public repository; default `develop`; `main` required checks `verify` and `develop-only`; PR required with zero approvals; administrators and conversation resolution enforced; force pushes and deletion disabled. Remote `develop` SHA `4d3f1c4314416137fb8d5ff9d2342404c488c90e` has successful CI run `33333315370`. Local `develop` is two commits ahead: rejected review SHA `9c66cc52e8c97a3b07642bcc0db0dd52e2ee4222` followed by the committed reviewer-finding correction. The correction awaits a new fixed-SHA review; neither local commit has been pushed or deployed.
