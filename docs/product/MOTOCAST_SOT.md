...
- Sixth-review working correction on 2026-09-01: the shared test fixture now creates a contract-valid schemaVersion 2 snapshot with twenty selected winding-only waypoints and three distinct safe routes. Vitest renders the real `SharedRideSnapshotView`, forces the real Kakao map child into SDK error, verifies all twenty omission rows and proves the notice is a sibling outside the fixed-height map. Production Chromium resolves that snapshot through the real public `/share` page at 320x800, 390x844, 820x1180, and 1440x900; before any scrolling it proves the notice and list have no internal overflow, the notice follows the map in the real snapshot tree, and the document has no horizontal overflow. Focused Vitest passes `13/13` and focused Chromium passes `10/10`. Earlier mounted-worktree cache failures remain `SETUP_OR_IMPORT_FAILURE 2`; the initial text-extraction authoring run remains `FAIL 2`; the first sandboxed local-port start remains `SETUP_OR_IMPORT_FAILURE 1`; and the first production-wiring assertion compared the wrong React wrapper level before correction, so it remains `FAIL 1`. Full writer verification, a new fixed SHA, six-axis delta review, hosted CI, and connected Preview remain pending. Production remains unchanged.
