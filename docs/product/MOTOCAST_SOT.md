...
- Current unreviewed correction: the trusted candidate policy now preserves the exact ordered selected waypoint list for balanced, winding, and shortest candidates. The live test verifies the persisted winding selection and, after deployment, will switch through all three candidates and require the winding marker plus completed selected-route weather for each. A synchronized writable copy passes `npm ci` (`407` packages, zero reported vulnerabilities), ESLint, TypeScript, Vitest (`50 files / 283 tests`), all five Deno entrypoints, deterministic Chromium (`14 PASS / 2 SKIP`), and the 13-route production build. One mounted-worktree TypeScript invocation remains `SETUP_OR_IMPORT_FAILURE 1`; one initially misaddressed writable-copy command remains `ERROR 1`; and the two connected live runs remain `FAIL 2`. A new fixed SHA, independent review, CI, Preview function deployment, and a successful connected live rerun are still required. Production remains unchanged.
...
- Rationale: Authenticated Preview smoke showed that this made a point the rider described as required disappear from two candidates and their maps. The user replaced it with the all-candidate requirement in `ROUTE-003`.
- User impact: New plans compare three strategies across the same rider-approved custom winding points. Older immutable share snapshots remain readable and truthfully label a historically omitted point without extending the provider geometry.
- Affected: trusted candidate policy, route/provider tests, map marker smoke, sharing compatibility, documentation.
- Verification: all three candidate policies retain the exact ordered winding point; actual Preview balanced, winding, and shortest routes expose the marker; finalization still rejects duplicate geometry.
