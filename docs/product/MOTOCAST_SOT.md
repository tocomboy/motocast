...
- Verification: exactly one provider invocation per split chunk; fixed safety parameters and absent `alternatives`; ordered mandatory point and occurrence-role matching; selected winding-only count/limit; dwell/return/24-hour boundaries; DB-side road distance/duration totals, numeric Korea-bounded vertices, `0.005` requested-point endpoint snap tolerance separated from `0.0002` road continuity, second-road/section and missing-field rejection, and finalization-time revalidation; one-draft atomic finalization and concurrency; one route row; legacy three-route finalization/read compatibility; schemaVersion 3 singular share plus schemaVersion 1/2 parsing; no candidate UI at mobile/desktop; real Preview route, geometry, ETA weather, collection, and share smoke.
