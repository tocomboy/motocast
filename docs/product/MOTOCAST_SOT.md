...
- GitHub: public repository; default `develop`; `main` required checks `verify` and `develop-only`; PR required with zero approvals; administrators and conversation resolution enforced; force pushes and deletion disabled. Remote `develop` SHA `4d3f1c4314416137fb8d5ff9d2342404c488c90e` has successful CI run `33333315370`. The HTTPS callback correction is currently an uncommitted local delta pending full verification and a new fixed-SHA review.
- Vercel: project `tocomboys-projects/motocast`, GitHub repository linked, Production Branch `main`, Node.js `20.x`, and `ssoProtection.deploymentType=preview`. Preview requires Vercel Authentication while Production remains outside that platform gate. Preview deployment `dpl_EA2gtau3512rd27dmY4s7E1rRZL7` is Ready at the fixed `develop` alias for SHA `4d3f1c4314416137fb8d5ff9d2342404c488c90e`; GitHub deployment `6170829596` and Vercel status both match that SHA. The unchanged import-time Production deployment remains Ready; no Production redeploy or promotion occurred.
...
- Supabase Preview: project `lehjmbgfpoemqcwxowbx` (`MOTOCAST_Preview`, Seoul `ap-northeast-2`) is `ACTIVE_HEALTHY`. Kakao Auth is enabled; Site URL and callback allowlist match the fixed `develop` Preview origin. All six migrations are applied. All 14 public application tables, including `kakao_oidc_handoffs`, have RLS enabled; service role has no direct table DML and exactly seven reviewed internal RPC grants. `search-places`, `plan-route`, `weather-timeline`, and `save-collection` remain active version 1 with JWT verification enabled; `kakao-oidc` is active version 1 with JWT verification disabled. Exactly ten user-defined Edge Function secret names are present, including the two OIDC names; values were not read or printed.
...
- Hosted OIDC runtime finding: the first actual Kakao browser login against Preview proved that Supabase Edge exposed an internal `http://...` `request.url`; the deployed start handler derived `redirect_uri` from that untrusted runtime representation, so Kakao rejected it as an unregistered URI even though the correct HTTPS callback was registered. The correction derives one exact callback URI from the trusted `SUPABASE_URL`, shares it between authorize and token exchange, and rejects non-loopback HTTP or base URLs containing credentials, a path, query, or fragment. The deployed broken version remains fail-closed and the correction requires a new fixed-SHA review and Preview redeployment before browser login resumes.
- Current OIDC correction revalidation: a fresh writable `/tmp` copy passed `npm ci` (`404` packages), full ESLint with zero warnings, TypeScript, Vitest (`42 files / 225 tests`), the Next.js Webpack production build, and all five Deno entrypoint checks. The two added assertions prove that an internal HTTP Edge request cannot influence the provider callback and that authorization and token exchange use the same trusted HTTPS callback. The exact approved local PostgreSQL 17 target at `127.0.0.1:54322` was read back before the immediately preceding DB run; Auth/RLS/plan/live-ACL suites passed `201/201`, and collection `4/4`, invite-budget-share `7/7`, route finalization `10/10`, and OIDC handoff `11/11` concurrency suites passed, for `233/233` DB assertions. No migration, DB test, RPC, RLS, route, weather, collection, or budget file changed after that run. `git diff --check` and the no-value-output sensitive-pattern scan passed before staging. Writer product results are `FAIL 0 / ERROR 0 / SKIP 0 / DESELECTED 0 / XFAIL 0`; environment-only setup starts remain recorded separately and every required path has a successful writable-copy or scoped-capability rerun. Hosted Preview and Production are unchanged.
...
- New fixed-SHA review and Preview redeployment of the trusted-HTTPS OIDC callback correction.
- Connected browser verification of the corrected direct OIDC flow, first-admin bootstrap, invitation login, and revoked-member denial.
