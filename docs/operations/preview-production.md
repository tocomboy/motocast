...
4. Push the exact reviewed release SHA only to a branch excluded by `vercel.json` (for example `review/single-route-<short-sha>`) and open a pull request into `develop` so hosted CI can run without deploying the incompatible Web first. Confirm branch SHA, CI results, and zero GitHub/Vercel deployments for the review branch. Do not merge this PR yet.
...
7. During the Preview maintenance window, deploy `search-places`, `plan-route`, `weather-timeline`, `save-collection`, and `kakao-oidc` from the reviewed fixed SHA. The first four retain JWT verification; only `kakao-oidc` is intentionally public with `verify_jwt=false` under `AUTH-004`. Read back each deployed Edge Function version and JWT setting, then merge the reviewed PR into `develop` to trigger the Web deployment last. Read back that exact merge SHA and Vercel Ready state before reopening route calculation.
