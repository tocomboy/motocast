...
A custom winding point is a rider-authored mandatory zero-dwell pass-through point, not an automatically detected route class. All provider chunks must retain its order together with meals and every selected rest occurrence. If Kakao cannot return a safe route through the approved points, the request fails explicitly; never request a winding alternative, generate an arbitrary detour, draw a straight line, or use a passenger-car fallback.
