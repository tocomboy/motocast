...
For the `PLAN-003` ordered-waypoint release, treat Web, `plan-route`, `save-collection`, and migration `20260902123000_ordered_waypoint_limits.sql` as one coupled Preview contract. The migration changes no table and rewrites no stored row; it replaces the two trusted validators so a forged `winding=false` value cannot bypass the semantic 20-waypoint limit. Both public Edge writers canonicalize accepted role-free pass-throughs to the legacy `winding=true` marker. After fixed-SHA review and non-deploying PR CI, dry-run and apply only that migration to the explicit Preview project, read back the migration head and affected function ACLs, deploy and read back only `plan-route` and `save-collection`, re-confirm the unchanged reviewed base, then fast-forward `develop` to the exact reviewed Web SHA. Route and collection mutations remain closed during the mixed-version interval. The authenticated gate must preserve a deliberately non-regrouped `lunch → waypoint → rest` sequence through both route requests, collection restore, and share legs, while retaining the existing weather, publication, cleanup, accessibility, and viewport gates. Production and `OPS-008` remain blocked.

Read-only release baseline on 2026-09-02: `origin/develop` is exact SHA `b94b3d706fdd1cb58fc39b508d3bd3e8592c13d0`; pull-request run `33537039488` and develop-push run `33537896753` both completed successfully for that SHA. GitHub Preview Deployment `6206653880` has successful status `17635834929` and points to the exact Vercel deployment. The explicit Preview project `lehjmbgfpoemqcwxowbx` has the first ten migrations through `20260901140000`; `20260902123000` is absent. Its active Edge baseline is `search-places` v8/JWT on, `plan-route` v7/JWT on, `weather-timeline` v8/JWT on, `save-collection` v7/JWT on, and `kakao-oidc` v8/JWT off. Do not use an implicit linked-project read: the local link can identify a different environment. This baseline is evidence only; the ordered-waypoint candidate still needs a new fixed SHA, independent review, CI-only PR with settled zero-deployment proof, and the coupled Preview sequence above.
...
3. Create the reader-compatible rollback Web artifact described above from the previous deployed application, independently review it, and record its exact SHA plus schemaVersion 1/2/3 read tests. Do not apply the Preview migration before this artifact exists.
   ...
   Prepared replacement artifact: remote slash-free branch `rollback-complete-course-reader-20260902`, exact SHA `b80edabcf7f46fe7cc473d014bcfdec779aca734`. It retains the superseded artifact's visibly closed collection controls and reads, fixed HTTP 503 `save-collection` handler, legacy new-trip writer, and immutable schemaVersion 1/2/3 parsers. Its Web reader additionally maps complete schemaVersion 3 route occurrences by validated order and accepts at most one microdegree of same-label coordinate drift on the legacy omission path, while preserving one-to-one historical `winding` roles and real `선택 경로 미통과` markers. Independent correctness, data-integrity, security, and operations reviews all report `BLOCKER 0 / HIGH 0 / MEDIUM 0 / LOW 0`. Writer evidence passes ESLint, TypeScript, full Vitest (`53 files / 298 tests`), all five Edge Function Deno checks, the 13-route production build, deterministic Chromium (`14 PASS / 2 SKIP`), and exact-range diff check. One backport test initially expected the current `경유지` terminology and failed `1/11`; after restoring the rollback artifact's historical `와인딩` contract, the focused suite passed `11/11` and the full evidence above passed. The remote ref read back the exact SHA; initial and 30-second settled GitHub readbacks both found zero Deployments, zero check-runs including Vercel, and zero Actions runs. This satisfies the prepared-artifact prerequisite for the current Preview rollout. During an actual rollback, deploy only this artifact's Web and Edge bundle to the already-migrated database and complete its live smoke before reopening collection actions.
