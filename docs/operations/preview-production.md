...
3. Create the reader-compatible rollback Web artifact described above from the previous deployed application, independently review it, and record its exact SHA plus schemaVersion 1/2/3 read tests. Do not apply the Preview migration before this artifact exists.
   ...
   Prepared replacement artifact: remote slash-free branch `rollback-complete-course-20260902`, exact SHA `0e09827fbd5dc67faaf9952ca6bdc28d49bf3f77`. It visibly closes all collection controls and collection reads in the connected Planner, and its deployed `save-collection` handler returns a fixed HTTP 503 without reading the body, authenticating, invoking an RPC, or touching a table. It never downgrades the ten-migration database. It preserves legacy new-trip route writing and immutable schemaVersion 1/2/3 reads, including an endpoint-only schemaVersion 3 share with no lunch while retaining the historical schemaVersion 1/2 required-lunch contract. Independent correctness, data-integrity, security, and operations reviews all report `BLOCKER 0 / HIGH 0 / MEDIUM 0 / LOW 0`. Writer evidence passes ESLint, TypeScript, full Vitest (`53 files / 296 tests`), all five Edge Function Deno checks, the 13-route production build, deterministic Chromium (`14 PASS / 2 SKIP`), and exact-range diff check. The remote ref read back the exact SHA; initial and 30-second settled GitHub readbacks both found zero Deployments, zero check-runs including Vercel, and zero Actions runs. This satisfies the prepared-artifact prerequisite for step 6. During an actual rollback, deploy only its Web and Edge bundle to the already-migrated database and complete the artifact's live smoke before reopening collection actions.
