...
3. **대기:** 작성자 및 lead 검사, 고정 SHA 독립 검토, 비배포 review 브랜치 exact-head CI 후 임시 함수 하나만 Preview에 배포한다. 동일 소유 ID/버전/갱신시각을 기록해 단 한 번 호출 후 삭제·부재·기존 다섯 함수 불변을 확인한다.
4. **대기:** 두 정시 요청의 정확 일치와 대상 시각 필수 값 결과로 후보 설계 가능 여부를 판정한다. 충분한 최신 선택·공개 지연·여섯 시간 끝의 기준을 확정한 후에만 제품 후보를 READY로 한다.
