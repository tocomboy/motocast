...
3. DONE: publish exact-SHA CI-only PR and verify no premature deployment.
4. DONE: align Vercel setting, promote verified SHA to Preview and read back build version and HTTP/auth boundary.
...
- Fixed candidate `9bcf151fa7af3ae8003fef4db980e1a5a11f1547`; [PR #76](https://github.com/tocomboy/motocast/pull/76) merged by exact-SHA fast-forward to develop after confirming the original base remained unchanged. [PR CI 36223797773](https://github.com/tocomboy/motocast/actions/runs/36223797773) PASS, Node 24.21.0, verify duration 2m17s. After CI settled, zero GitHub Deployments and zero Vercel statuses/checks were confirmed before promotion.
- [Post-promotion develop CI 36223950194](https://github.com/tocomboy/motocast/actions/runs/36223950194) PASS for the same exact SHA, 2m14s. No additional runtime matrix or duplicate PR-branch push trigger was introduced.
- Vercel project setting saved as 24.x. The Node 20 EOL warning disappeared; existing Production runtime override remains visible because that deployment has not been replaced.
- Preview `dpl_8VQbhUTnBqKB12RLXtQrAVKcsn8G`, GitHub deployment `6675615392`: READY / success, source develop at exact `9bcf151...`, 51 seconds, completed 2026-09-26 06:31:15 UTC. Actual build output v24.21.0; deployment Runtime Settings 24.x. Fixed develop domain assigned successfully.
- No-cookie HTTP readback: login 200 with Kakao UI, share shell 200, admin redirect 307 to login, anonymous private-course 401, synthetic unknown share 404, separate protected Preview URL redirects 302 to Vercel. All six boundary probes PASS. The still-absent assetlinks endpoint returns 404 as expected from the current released code; App Links are NOT_READY, not counted as functional PASS.
- Original Production remains `259f4866a0cb96c33e84423a048d385aacda65f0` / deployment `dpl_9GuPPNhKsVNeq6xnitwfmKDWS1nJ`. The main branch still pins Node 20 until a separately gated develop-to-main promotion. A project setting alone does not override `package.json`; do not claim Production runtime migration complete.
- No new tools installed. Notion external recording remains pending because tool access was unavailable; repository and PR evidence are retained.
- Integration into pending PR #75 preserves its public tester-origin exception and Play changes. The one operations-document conflict was resolved by retaining both the 24.x runtime and OPS-005 exception; no product code conflict occurred. The previous 6826174 base is superseded by 9bcf151 for future exact-SHA promotion.
