...
4. CI-only PR #60 and initial Preview release: COMPLETE for `1dd0997a199b6d8af848e494a6f8565b8da7c0cd`; full Preview gate remains IN_PROGRESS. Initial actual smoke found the empty-map presentation regression recorded as UI-REV-16; the corrected candidate must complete the same exact-SHA CI/Preview sequence.
...
6. Back up and delete the confirmed all-user old-history scope while preserving accounts/memberships: Preview COMPLETE (118 backed-up target rows removed; accounts 2 unchanged). Production remains AUTHORIZED, NOT_RUN (119-row backup retained).
...
## Initial Preview release execution

On 2026-09-17, PR #60 reviewed exact head `1dd0997a199b6d8af848e494a6f8565b8da7c0cd` against base `c2abc4c2752e39273dc0435378e7747e1df6ef52`. GitHub CI run `35133013673` completed successfully on that head. Initial and settled readbacks found zero GitHub Deployments and no Vercel check/status before any hosted mutation. The PR was held open.

After a fresh exact-backup dry run, the pinned operator made one successful Preview apply attempt. Independent readback found all eleven target history tables empty and accounts still 2. CLI 2.116.0 then applied exactly migrations `20260916150133` and `20260916155040`; migration readback confirms all fourteen timestamps. Preview `plan-route` v31 is ACTIVE, JWT verification true, archive SHA-256 `8085f24a977716b8f59f8d06bac213e054d00c3adf6569b328d00ed2f4eb79ad`. All fourteen downloaded source files match the fixed commit payload. Linked live ACL checks passed 155/155. The initial shell PATH quoting error occurred before CLI execution and made no migration attempt; the subsequent pinned CLI invocation completed normally.

Immediately before Web release, `origin/develop` still equalled the reviewed base. A normal fast-forward push made develop equal the exact reviewed head. Vercel Preview `dpl_8XPeELsAVmqVM37oJih1ZMgcCcBD` is READY for that develop SHA; GitHub Deployment `6487532489` records Preview/success. Post-push CI `35133436586` also passed. Production remains on `65c467f19d0ea39807123a0379d811ba61b72011`; Production history, migrations and Edge artifact have not been modified in this rollout.
