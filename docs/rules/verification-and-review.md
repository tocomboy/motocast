...
npm run lint
npm run typecheck
npx --yes deno check supabase/functions/search-places/index.ts supabase/functions/plan-route/index.ts supabase/functions/weather-timeline/index.ts supabase/functions/save-collection/index.ts supabase/functions/kakao-oidc/index.ts supabase/functions/play-admission/index.ts
npm test
...
- Received-course reuse and favorites: exercise authenticated active recipient copy and private planning bootstrap, recipient ownership, no copied departure schedule, public proof nonexposure, revoked/unknown share and inactive-member denial, immutable copied courses, exact operation retry and concurrent copy. Shared-page navigation must keep the bearer and private course in component memory and isolate replacement fragments and detached planners. Run `place_favorites.test.sql` and `place_favorites_concurrency.test.sql` on the verified disposable local database: owner-only reads, direct-write/service-role denial, malformed-place rejection, exactly three slots, duplicate retries, stale exact-delete rejection, concurrent fourth-slot rejection and concurrent duplicate idempotency. New mobile/desktop flows require current rendered dialog visibility, normal/error/cancel navigation, focus return and unclipped readable text; keyless mocks do not replace connected provider and ownership checks.
