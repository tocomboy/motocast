...
- Sharing: preview, publish, immutable source edit, revoke, reissue, unknown/revoked token, cross-user management denial, and previously emitted schemaVersion 1 fixture compatibility. A collection-card share entry must apply the complete origin/destination/ordered-point course, complete fresh route and weather persistence, open the concise route-and-weather preview exactly once across React Strict Mode replay, and still require a separate explicit publish action.
